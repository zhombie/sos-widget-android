package kz.gov.mia.sos.widget.core.device
...
import kz.garage.context.system.service.telephonyManager
import kz.garage.context.system.service.wifiManager
import java.io.File
import java.util.*
...
class Device constructor(private val context: Context) {
    ...
    val ringerMode: String
        get() {
            return when (context.audioManager?.ringerMode) {
    ...
    val phoneType: String
        get() {
            return when (context.telephonyManager?.phoneType) {
    ...
    val isSimNetworkLocked: Boolean
        get() {
            return context.telephonyManager?.simState == TelephonyManager.SIM_STATE_NETWORK_LOCKED
        }
    ...
    val isWifiEnabled: Boolean
        get() {
            return context.wifiManager?.isWifiEnabled == true
        }
