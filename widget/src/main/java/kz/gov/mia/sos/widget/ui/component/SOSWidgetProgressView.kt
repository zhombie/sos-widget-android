...
internal class SOSWidgetProgressView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        val view = inflate(context, R.layout.sos_widget_view_progress, this)

        rootLayout = view.findViewById(R.id.rootLayout)
        centerLayout = view.findViewById(R.id.centerLayout)
        cancelButton = view.findViewById(R.id.cancelButton)

        context.obtainStyledAttributes(attrs, R.styleable.SOSWidgetProgressView).use { typedArray ->
            rootLayout.setBackgroundColor(
                typedArray.getColor(
                    R.styleable.SOSWidgetProgressView_sos_widget_background_color,
                    ContextCompat.getColor(context, R.color.sos_widget_black_alpha)
                )
            )
            ...
            when (typedArray.getInt(
                R.styleable.SOSWidgetProgressView_sos_widget_type,
                Type.INDETERMINATE
            )) {
            ...
            isCancelable = typedArray.getBoolean(
                R.styleable.SOSWidgetProgressView_sos_widget_is_cancelable,
                false
            )
        }
    ...
    private fun createProgressIndicator(style: Int): CircularProgressIndicator =
        CircularProgressIndicator(context, null, style).apply {
            ...
            layoutParams = MarginLayoutParams(
                context.resources.getDimensionPixelOffset(R.dimen.sos_widget_progress_circle_size),
                context.resources.getDimensionPixelOffset(R.dimen.sos_widget_progress_circle_size)
            )
            ...
            indicatorSize =
                context.resources.getDimensionPixelOffset(R.dimen.sos_widget_progress_circle_size)
            trackThickness =
                context.resources.getDimensionPixelOffset(R.dimen.sos_widget_progress_track_thickness)

            setIndicatorColor(ContextCompat.getColor(context, R.color.sos_widget_white))

            trackColor = ContextCompat.getColor(context, R.color.sos_widget_transparent)
        }
    ...
    private fun createTextView(): MaterialTextView =
        MaterialTextView(context, null, android.R.attr.textViewStyle, 0).apply {
            ...
            maxWidth =
                context.resources.getDimensionPixelOffset(R.dimen.sos_widget_progress_text_max_width)
